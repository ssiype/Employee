...
namespace MyEmployeeDirectory
{
    public partial class EmployeeDirectory : Form
    {
       ...
       EmployeeInfo _empinfo;
            ...
            try
            {
                using (EmployeeContext _empcntx = new EmployeeContext())
                {
                    ...
                    if (txtBxName.Text != "")
                    {
                        ...
                        empcntxt.SaveChanges();//Saving the added employee record
                        refreshGridView();
                        refreshDataGridView();
        ...
        {
            //The id of the table is unique and a record is identified based on the id 
        ...
        private void dataGridView1_DoubleClick(object sender, EventArgs e)
        {
            if (dataGridView1.CurrentRow.Index >= 0)
            {
                UpdatextBox();//populate the textboxes with selected data grid row
